...
public final class HighwayGraph {
    ...
    private static List<List<Integer>> buildAdjacency(int n, List<Edge> edges) {
        ...
        for (int i = 0; i < n; i++) adj.set(i, Collections.unmodifiableList(adj.get(i)));
        return Collections.unmodifiableList(adj);
    }

    public int vertexCount()              { return vertices.size(); }
    public int edgeCount()                { return edges.size(); }
    public Vertex vertex(int index)       { return vertices.get(index); }
    public List<Vertex> vertices()        { return vertices; }
    public List<Edge> edges()             { return edges; }
    public List<Integer> incidentEdges(int v) { return adjacencyByEdgeIndex.get(v); }
}
